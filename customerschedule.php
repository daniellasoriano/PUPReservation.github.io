...
<html lang = "en">
  <head>
		...
		<title>PUP-Reservation Schedule (Customer's Name)</title>
			...
			<div class = "modal-header" style="background:#f0f5f5;">
				<font size="6">Reservation Schedule</font>
					...
					<div class = "modal-body">
							...
							<input readonly type="text" class="form-control" name="search" placeholder="Customer's Name">
							</div>
							<label>Search:</label>
							<form method = 'post'>
							<div class="inline text-center ">
									<input type="text" class="form-control" name="custname" placeholder="Customer's Name">
...
<?php
 ...
 include('connection.php');
  ...
  $sCust = mysqli_query($con, "Select * from tblCustomer");
  while($row=mysqli_fetch_array($sCust)){
  	if($customer == $row['strCustName']){
  		 $searchCust = $row['intCustID'];
  	}
  }

  $sqlreserv = mysqli_query($con, "Select * from tblreservation where intCustID_FK = $searchCust order by dtmReservationDate");
